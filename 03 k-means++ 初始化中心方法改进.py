...
le=np.shape(data)[0]
# 随机选取一个初始化中心
firmean=data[np.random.randint(le)]
def secmean(data,firmean,dist=distEclud):
    maxdist=0;index=-1
    for i in range(le):
    # 计算样本与均值向量的距离，选择距离最大的样本为第二个簇中心
        ...
        if disti > maxdist:
            ...
            index=i
    return data[index]
fsmean=np.vstack((firmean,secmean(data,firmean))) 
